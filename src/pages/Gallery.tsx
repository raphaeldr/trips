...
const Gallery = () => {
  ...
  const filteredPhotos = photos
    ?.filter((photo) => {
      ...
      if (searchQuery) {
        ...
        const matchesTags = photo.ai_tags?.some((tag) =>
  ...
  const handleAutoTagAll = async () => {
    ...
    for (const photo of untaggedPhotos) {
      try {
        const { error } = await supabase.functions.invoke("tag-photo", {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 container mx-auto px-6 py-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          ...
          {photos && photos.some((p) => !p.ai_tags || p.ai_tags.length === 0) && (
            <Button onClick={handleAutoTagAll} className="gap-2">
