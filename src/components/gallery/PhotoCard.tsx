...
export const PhotoCard = ({
  ...
  const handleSetAsHero = async (e: React.MouseEvent) => {
    ...
    try {
      const { error } = await supabase.from("moments").update({ is_hero: !isHero }).eq("id", id);
  ...
  return (
    <>
      <div className={`group cursor-pointer flex flex-col gap-2 ${className || ""}`} onClick={() => setIsOpen(true)}>
        <div className={`relative w-full overflow-hidden rounded-2xl bg-gray-100 ${className?.includes("aspect-") ? "" : "aspect-square"}`}>
          {isVideo ? (
            ...
            <img
              src={imageError ? "/placeholder.svg" : publicUrl}
              alt={title || "Travel photo"}
              className={`w-full h-full object-cover transition-all duration-500 will-change-transform group-hover:scale-[1.02] group-hover:brightness-90 ${imageLoaded || imageError ? "opacity-100" : "opacity-0"
        ...
        {/* Hidden text for minimal look, or make very subtle */}
