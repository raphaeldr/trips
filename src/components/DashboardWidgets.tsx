      ...
      <div className="flex-1 p-5 md:p-6 flex flex-col min-h-0 bg-card relative">
        ...
        <ScrollArea className="flex-1 -mx-4 px-4 bento-scroll h-full">
          <div className="space-y-0 relative z-0 pb-2">
            ...
            {destinations?.map((dest, i) => <div key={dest.id} className="relative pl-8 py-2.5 md:py-3 group/item cursor-default">
                <div className={`absolute left-4 top-[14px] md:top-[18px] w-1.5 h-1.5 rounded-full ring-4 ring-card ${i === 0 ? "bg-primary scale-125" : "bg-muted-foreground/30 group-hover/item:bg-primary/50"} transition-all duration-300 z-10`} />
                <div className="flex flex-col">
                  <span className={`text-sm font-medium leading-none ${i === 0 ? "text-foreground" : "text-muted-foreground group-hover/item:text-foreground"} transition-colors`}>
                    {dest.name}
                  </span>
