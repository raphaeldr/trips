...
export const HeroPhotoManager = () => {
  ...
  const heroMoment = moments?.find(p => p.is_hero);

  const handleSetHero = async (momentId: string) => {
    try {
      // First, unset all hero moments
      await supabase
        .from('moments')
        .update({ is_hero: false })
        .neq('id', '00000000-0000-0000-0000-000000000000');

      // Then set the new hero moment
  ...
  // Filter to only show photos/videos (not audio/text)
  const visualMoments = moments?.filter(m => m.media_type === 'photo' || m.media_type === 'video' || !m.media_type);
  ...
  return (
    <div className="space-y-6">
      {heroMoment ? (
        <div className="space-y-4">
          ...
          <div className="relative aspect-video rounded-lg overflow-hidden bg-muted">
            {heroMoment.animated_path ? (
              <video
                src={`${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/photos/${heroMoment.animated_path}`}
                ...
                playsInline
              />
            ) : heroMoment.storage_path ? (
              <img
                src={`${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/photos/${heroMoment.storage_path}`}
                alt={heroMoment.title || 'Hero photo'}
                className="w-full h-full object-cover"
              />
            ) : null}
          </div>
      ...
      {visualMoments && visualMoments.length > 0 && (
        <div className="space-y-2">
          <Label className="text-sm font-medium">All Photos & Videos</Label>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto p-2">
            {visualMoments.map((moment) => (
              <div key={moment.id} className="relative group">
                {moment.storage_path && (
                  <img
                    src={`${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/photos/${moment.storage_path}`}
                ...
                <Button
                  size="sm"
                  variant={moment.is_hero ? "default" : "secondary"}
                  className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity"
                  onClick={() => handleSetHero(moment.id)}
                >
                  {moment.is_hero ? (
